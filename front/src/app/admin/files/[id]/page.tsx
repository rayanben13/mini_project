  ...
  return (
    <div className="relative min-h-screen bg-background">
      ...
      {fileDetails?.status === "pending" && (
        <div className="sticky top-17 z-[60] flex justify-end px-4 sm:px-6 py-1">
          <div className="w-fit max-w-[420px] border-b border-slate-200/70 dark:border-slate-800/70 bg-white/80 dark:bg-slate-950/80 backdrop-blur-lg rounded-2xl px-3 py-2 flex items-center gap-2 shadow-sm">
            {/* Reject */}
            <Button
              onClick={() => setIsRejectOpen(true)}
              variant="outline"
              disabled={approveMutation.isPending || rejectMutation.isPending}
              className="h-11 rounded-2xl border-red-200 dark:border-red-900/40 bg-red-50/50 dark:bg-red-950/20 text-red-600 hover:bg-red-100 dark:hover:bg-red-950/30 hover:text-red-700 transition-all duration-200 px-5 shadow-sm"
            >
            ...
            <Button
              ...
              disabled={approveMutation.isPending || rejectMutation.isPending}
              className="h-11 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white px-5 shadow-lg shadow-emerald-500/20 transition-all duration-200"
            >
