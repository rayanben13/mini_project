...
const SubjectCard = memo(
  ({ subject, index }: { subject: any; index: number }) => {
      ...
      >
        ...
        <div className="pt-5 mt-4 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs text-slate-400 font-medium">
            <span>
              {subject._count?.files ?? 0}{' '}
              {subject._count?.files === 1 ? 'File' : 'Files'}
            </span>
            <span className="text-slate-200 dark:text-slate-700">|</span>
            <span>
              {subject._count?.study_lists ?? 0}{' '}
              {subject._count?.study_lists === 1 ? 'List' : 'Lists'}
            </span>
          </div>
