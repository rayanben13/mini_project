...
import { CreateStudyListModal } from '@/app/(dashboard)/dashboard/study-list/CreateStudyListModal';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function SaveStudyListModal({
  isOpen,
  onClose,
  fileId,
}: SaveModalProps) {
  ...
  const { createStudyList, loading: isCreatingList } = useStudyListStore();
  ...
  const handleCreate = async (data: any) => {
    ...
    const res = await createStudyList(dataConfig);
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const response = await saveFileToStudyList(
                          fileId,
                          list.id_stuList
                        );
                        ...
                        if (response.success) {
                          toast.success(
                            ...
                            {
                              description:
                                'Your file has been saved to your study list.',
                              duration: 3000,
                            }
                          );
