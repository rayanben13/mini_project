    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button
                    variant="outline"
                    className={className || `gap-2 rounded-xl h-10 ${isActive
            ...
            <DialogContent showCloseButton={false} className="sm:max-w-[340px] rounded-[24px] p-5 sm:p-6 bg-white dark:bg-slate-900 border-none shadow-2xl">
                ...
                <div className="space-y-4">
                    {/* Calendar Section */}
                    <div>
                        <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">Select Date</p>
                        <div className="border border-slate-200 dark:border-slate-800 rounded-xl p-3">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <div className="flex gap-1">
                                    <button onClick={handlePrevMonth} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md text-slate-500 transition-colors">
                                        <ChevronLeft className="w-4 h-4" />
                                    </button>
                                    <button onClick={handleNextMonth} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md text-slate-500 transition-colors">
                            ...
                            <div className="grid grid-cols-7 gap-y-1">
                                {days.map((day, idx) => {
                                    if (day === null) {
                                        ...
                                        return (
                                            <div key={`prev-${idx}`} className="h-7 w-7 mx-auto flex items-center justify-center text-xs font-medium text-slate-300 dark:text-slate-600">
                                                {prevDay}
                                            </div>
                                        );
                                    }
                                    if (day > 100) {
                                        ...
                                        return (
                                            <div key={`next-${idx}`} className="h-7 w-7 mx-auto flex items-center justify-center text-xs font-medium text-slate-300 dark:text-slate-600">
                                                {nextDay}
                                            </div>
                                        );
                                    }
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Hour</p>
                            <Select value={hour} onValueChange={setHour}>
                                ...
                                <SelectContent position="popper" sideOffset={4} className="rounded-lg max-h-[200px] min-w-[100px]">
                                    {Array.from({ length: 24 }).map((_, i) => {
                                        const val = String(i).padStart(2, '0');
                                        return <SelectItem key={val} value={val} className="text-sm rounded-md">{val}</SelectItem>
                                    })}
                                </SelectContent>
                            </Select>
                        </div>
                        <div>
                            <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Minute</p>
